...
async def clinical_chat(
    ...
    try:
        ...
        # NEW: same before/after timing log as rag_engine.py's report
        # generation, so a stalled chat call shows up in the terminal
        # instead of the request just silently sitting there.
        print(f"DEBUG: sending chat request to OpenRouter for: {request.question!r}")
        ...
        try:
            ...
            print(f"DEBUG: OpenRouter chat call FAILED after {elapsed:.1f}s: {exc!r}")
            raise
        elapsed = time.monotonic() - start
        print(f"DEBUG: OpenRouter chat call completed in {elapsed:.1f}s, {len(answer)} chars")
